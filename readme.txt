...
# General steps to build project:
...
Remove -flto from the build options, as this resulted in errors in the LTO component related to comdat sections. Also compiling with libraries for ASan has not been tested yet.
...
To start with MSan, use add_build_options.sh to add the relevant library files. Make sure VariableSmm.inf contains the right build options then VariableSmm.efi should be instrumented. Then use build.sh and qemu.sh to build and run.
